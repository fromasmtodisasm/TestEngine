...
int CWeaponClass::Fire(const Legacy::Vec3& origin, const Legacy::Vec3& angles, CPlayer* pPlayer, WeaponInfo& winfo, IPhysicalEntity* pIRedirected)
{
	...
	if (m_fireParams.whizz_sound_radius)
	{
		...
		if (pClient)
		{
			...
			if (pEntSys)
			{
				...
				{
					Legacy::Vec3 ShooterPlayerVec=pPlayerEntity->GetPos()-pPlayer->GetEntity()->GetPos();
					float fDot=ShooterPlayerVec.Dot(dir);
					Legacy::Vec3 BulletPlayerPos=dir*fDot+pPlayer->GetEntity()->GetPos();

					float fDist2 = GetLengthSquared(Legacy::Vec3(BulletPlayerPos-pPlayerEntity->GetPos()));
	...
	if (m_fireParams.iFireModeType == FireMode_Projectile)
	{
		...
		// Weapon not ready to fire ?
		{
			FRAME_PROFILER( "CWeaponClass::ScriptEvent_Fire",PROFILE_GAME );
		...
		for (int bullet = 0; bullet < m_fireParams.nBulletpershot; bullet++)
		{
			...
			{
				FRAME_PROFILER( "CWeaponClass::Launch",PROFILE_GAME );
	...
	#define MAX_HITS 5
		...
		{
			FRAME_PROFILER( "CWeaponClass::FireMode_Melee",PROFILE_GAME );
			// center of the 'hit box' of the melee weapon
			Legacy::Vec3 boxCenter = origin + 0.5f * dir;
			Legacy::Vec3 offset(0.5f * dir.Length(), 0.5f * dir.Length(), 0.5f * dir.Length());
		...
		Legacy::Vec3 currangles= weapangles;
		Legacy::Vec3 currpos	= origin;

		// First bullet ?
		if (bFirstBullet)
		{
			FRAME_PROFILER( "CWeaponClass::FirstBullet_ScriptOnFire",PROFILE_GAME );
			...
			m_ssoHitPt = (Legacy::Vec3) hits[0].pt;
		...
		{
			FRAME_PROFILER( "CWeaponClass::BulletLoop",PROFILE_GAME );
			...
			IEntityRender	*entrendercontact=NULL;
				...
				if (res && hits[nCount].dist>0 && hits[nCount].pCollider)
				{
					...
					if (physType == OT_BOID)
					{
						...
						{
							string surfaceName;
							Legacy::Vec3 vImpulse = GetNormalized(dir)*float(m_fireParams.iImpactForceMul)*float(m_fireParams.iImpactForceMulFinal);
					...
					{
						Legacy::Vec3 vImpulse = GetNormalized(dir)*float(m_fireParams.iImpactForceMul);
						// Add impulse to it.
				...
				SWeaponHit hit;
				hit.pos = (Legacy::Vec3)hits[nCount].pt;
				hit.dir = GetNormalized(dir);
				hit.normal = (Legacy::Vec3)hits[nCount].n;
	...
	{
		FRAME_PROFILER( "CWeaponClass::AutoBalance",PROFILE_GAME );
...
void CWeaponClass::ProcessHit(const SWeaponHit& hit)
{
	...
	{
		Legacy::Vec3 vWaterNormal(0,0,1);
		Legacy::Vec3 vTarget=hit.pos;
		Legacy::Vec3 vNorm;
