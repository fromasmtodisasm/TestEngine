...
public:
	...
	template<typename T>
	void AddObject(const _smart_ptr<T>& rObj)
	{
	...
	void AddObject(const string& rString) { this->Add(rString.c_str(), rString.capacity()); }
...
#define SIZER_COMPONENT_NAME(pSizerPointer, szComponentName) \
	PREFAST_SUPPRESS_WARNING(6246)                           \
	CrySizerComponentNameHelper __sizerHelper(pSizerPointer, szComponentName, false)
#define SIZER_SUBCOMPONENT_NAME(pSizerPointer, szComponentName) \
	PREFAST_SUPPRESS_WARNING(6246)                              \
	CrySizerComponentNameHelper __sizerHelper(pSizerPointer, szComponentName, true)