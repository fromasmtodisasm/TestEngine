...
#define ccc(var) const_cast<char*>(var)
...
namespace Detail
{
	...
	struct SDummyCVar : ICVar
	{
		...
		~SDummyCVar()
		{
			::ConsoleRegistrationHelper::Unregister(this);
		}
		...
		{
			if (SQueryTypeEnum<T>::ParseString(s) != value) InvalidAccess();
		}
		void ForceSet(const char* s) override { Set(s); }
		void Set(float f) override
		{
			if (static_cast<T>(f) != value) InvalidAccess();
		}
		void Set(int i) override
		{
			if (static_cast<T>(i) != value) InvalidAccess();
		}
		void Set(int64 i) override
		{
			if (static_cast<T>(i) != value) InvalidAccess();
		}
		...
		const SmallFunction<void()>& GetOnChangeFunctor(uint64 nFunctorIndex) const override
		{
			InvalidAccess();
			static SmallFunction<void()> oDummy;
			return oDummy;
		}
	...
	#define DeclareConstIntCVar(name, defaultValue) enum : int \
	{                                                          \
		name = GetCVarOverride(#name, defaultValue)            \
	}
	#define DeclareStaticConstIntCVar(name, defaultValue) enum : int \
	{                                                                \
		name = GetCVarOverride(#name, defaultValue)                  \
	}
	#define DefineConstIntCVarName(strname, name, defaultValue, flags, help)                                                      \
		{                                                                                                                         \
			static_assert(static_cast<int>(GetCVarOverride(#name, defaultValue)) == static_cast<int>(name), "Unexpected value!"); \
			REGISTER_DUMMY_CVAR(int, strname, GetCVarOverride(#name, defaultValue));                                              \
		}
	#define DefineConstIntCVar(name, defaultValue, flags, help)                                                                   \
		{                                                                                                                         \
			static_assert(static_cast<int>(GetCVarOverride(#name, defaultValue)) == static_cast<int>(name), "Unexpected value!"); \
			REGISTER_DUMMY_CVAR(int, (#name), GetCVarOverride(#name, defaultValue));                                              \
		}
...
//! DefineConstIntCVar2 is deprecated, any such instance can be converted to the 3 variant by removing the quotes around the first parameter.
	#define DefineConstIntCVar3(name, _var_, defaultValue, flags, help)                                                            \
		{                                                                                                                          \
			static_assert(static_cast<int>(GetCVarOverride(#name, defaultValue)) == static_cast<int>(_var_), "Unexpected value!"); \
			REGISTER_DUMMY_CVAR(int, name, GetCVarOverride(#name, defaultValue));                                                  \
		}
	...
	#define DefineConstFloatCVar(name, flags, help)                                     \
		{                                                                               \
			REGISTER_DUMMY_CVAR(float, (#name), GetCVarOverride(#name, name##Default)); \
		}
...
#define SAFE_UNREGISTER_CVAR(pVar)                   \
	if (pVar)                                        \
	{                                                \
		ConsoleRegistrationHelper::Unregister(pVar); \
		pVar = nullptr;                              \
	}
...
#if defined(RELEASE)
	#define REGISTER_CVAR_DEV_ONLY(_var, _def_val, _flags, _comment)                                       \
		NULL;                                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!"); \
		_var = _def_val
	#define REGISTER_CVAR_CB_DEV_ONLY(_var, _def_val, _flags, _comment, _onchangefunction)                 \
		NULL;                                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!"); \
		_var = _def_val /* _onchangefunction consumed; callback not available */
	#define REGISTER_STRING_DEV_ONLY(_name, _def_val, _flags, _comment) \
		NULL;                                                           \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!") /* consumed; pure cvar not available */
	#define REGISTER_STRING_CB_DEV_ONLY(_name, _def_val, _flags, _comment, _onchangefunction) \
		NULL;                                                                                 \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!") /* consumed; pure cvar not available */
	#define REGISTER_INT_DEV_ONLY(_name, _def_val, _flags, _comment) \
		NULL;                                                        \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!") /* consumed; pure cvar not available */
	#define REGISTER_INT_CB_DEV_ONLY(_name, _def_val, _flags, _comment, _onchangefunction) \
		NULL;                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!") /* consumed; pure cvar not available */
	#define REGISTER_INT64_DEV_ONLY(_name, _def_val, _flags, _comment) \
		NULL;                                                          \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!") /* consumed; pure cvar not available */
	#define REGISTER_FLOAT_DEV_ONLY(_name, _def_val, _flags, _comment) \
		NULL;                                                          \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!") /* consumed; pure cvar not available */
	#define REGISTER_CVAR2_DEV_ONLY(_name, _var, _def_val, _flags, _comment)                               \
		NULL;                                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!"); \
		*(_var) = _def_val
	#define REGISTER_CVAR2_CB_DEV_ONLY(_name, _var, _def_val, _flags, _comment, _onchangefunction)         \
		NULL;                                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!"); \
		*(_var) = _def_val
	#define REGISTER_CVAR3_DEV_ONLY(_name, _var, _def_val, _flags, _comment)                               \
		NULL;                                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!"); \
		_var = _def_val
	#define REGISTER_CVAR3_CB_DEV_ONLY(_name, _var, _def_val, _flags, _comment, _onchangefunction)         \
		NULL;                                                                                              \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!"); \
		_var = _def_val
...
#else
	#define REGISTER_CVAR_DEV_ONLY(_var, _def_val, _flags, _comment)       \
		REGISTER_CVAR(_var, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR_CB_DEV_ONLY(_var, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_CVAR_CB(_var, _def_val, ((_flags) | VF_DEV_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_STRING_DEV_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_STRING(_name, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_STRING_CB_DEV_ONLY(_name, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_STRING_CB(_name, _def_val, ((_flags) | VF_DEV_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_INT_DEV_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_INT(_name, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_INT_CB_DEV_ONLY(_name, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_INT_CB(_name, _def_val, ((_flags) | VF_DEV_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_INT64_DEV_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_INT64(_name, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_FLOAT_DEV_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_FLOAT(_name, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR2_DEV_ONLY(_name, _var, _def_val, _flags, _comment)       \
		REGISTER_CVAR2(_name, _var, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR2_CB_DEV_ONLY(_name, _var, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_CVAR2_CB(_name, _var, _def_val, ((_flags) | VF_DEV_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR3_DEV_ONLY(_name, _var, _def_val, _flags, _comment)       \
		REGISTER_CVAR3(_name, _var, _def_val, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR3_CB_DEV_ONLY(_name, _var, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_CVAR3_CB(_name, _var, _def_val, ((_flags) | VF_DEV_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_COMMAND_DEV_ONLY(_name, _func, _flags, _comment)       \
		REGISTER_COMMAND(_name, _func, ((_flags) | VF_DEV_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
...
//! (2) Registered as real cvars in release on dedi servers only, otherwise treated as DEV_ONLY type cvars (see above)
	...
	#define REGISTER_CVAR_DEDI_ONLY(_var, _def_val, _flags, _comment)       \
		REGISTER_CVAR(_var, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR_CB_DEDI_ONLY(_var, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_CVAR_CB(_var, _def_val, ((_flags) | VF_DEDI_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_STRING_DEDI_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_STRING(_name, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_STRING_CB_DEDI_ONLY(_name, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_STRING_CB(_name, _def_val, ((_flags) | VF_DEDI_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_INT_DEDI_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_INT(_name, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_INT_CB_DEDI_ONLY(_name, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_INT_CB(_name, _def_val, ((_flags) | VF_DEDI_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_INT64_DEDI_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_INT64(_name, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_FLOAT_DEDI_ONLY(_name, _def_val, _flags, _comment)       \
		REGISTER_FLOAT(_name, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR2_DEDI_ONLY(_name, _var, _def_val, _flags, _comment)       \
		REGISTER_CVAR2(_name, _var, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR2_CB_DEDI_ONLY(_name, _var, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_CVAR2_CB(_name, _var, _def_val, ((_flags) | VF_DEDI_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR3_DEDI_ONLY(_name, _var, _def_val, _flags, _comment)       \
		REGISTER_CVAR3(_name, _var, _def_val, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_CVAR3_CB_DEDI_ONLY(_name, _var, _def_val, _flags, _comment, _onchangefunction)       \
		REGISTER_CVAR3_CB(_name, _var, _def_val, ((_flags) | VF_DEDI_ONLY), _comment, _onchangefunction); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
	#define REGISTER_COMMAND_DEDI_ONLY(_name, _func, _flags, _comment)       \
		REGISTER_COMMAND(_name, _func, ((_flags) | VF_DEDI_ONLY), _comment); \
		static_assert((_flags & ILLEGAL_DEV_FLAGS) == 0, "Flags must not contain any development flags!")
#else
