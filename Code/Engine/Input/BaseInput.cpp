...
{
	assert(pListenerA != NULL && pListenerB != NULL);
...
uint32_t CBaseInput::GetInputCharUnicode(const SInputEvent& event)
{
	for (TInputDevices::iterator i = m_inputDevices.begin(); i != m_inputDevices.end(); ++i)
	{
...
void CBaseInput::RemoveDeviceHoldSymbols(EInputDeviceType deviceType, uint8_t deviceIndex)
{
...
bool CBaseInput::ShouldBlockInputEventPosting(const EKeyId keyId, const EInputDeviceType deviceType, const uint8_t deviceIndex) const
{
...
void CBaseInput::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
{
	...
	//{
	//	//LogAlways("BaseInput - Shutting down all force feedback");
	//}
}
...
IInputDevice* CBaseInput::GetDevice(uint16_t id, EInputDeviceType deviceType)
{
