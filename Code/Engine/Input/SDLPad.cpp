...
void CSDLPad::Update(bool bFocus)
{
	...
	{
		m_vibrateTime -= GetISystem()->GetITimer()->GetFrameTime();
...
bool CSDLPad::SetForceFeedback(IFFParams params)
{
	...
	if (m_supportsFeedback && m_curHapticEffect < 0)
	{
		...
		effect.leftright.large_magnitude = (uint16_t)(clamp_tpl(params.strengthB, 0.0f, 1.0f) * 65536.0f);
		effect.leftright.small_magnitude = (uint16_t)(clamp_tpl(params.strengthA, 0.0f, 1.0f) * 65536.0f);
		...
		{
			GetISystem()->GetILog()->LogError("CSDLPad - Gamepad [%d] failed to create feedback effect: %s", m_deviceNo, SDL_GetError());
			return false;
		}
		GetISystem()->GetILog()->Log("CSDLPad - Gamepad [%d] submitting create feedback effect (%d,%d,%d)", m_deviceNo, effect.leftright.large_magnitude, effect.leftright.small_magnitude, effect.leftright.length);
...
void CSDLPad::HandleAxisEvent(const SDL_JoyAxisEvent& axisEvt)
{
	...
	if (pSymbol)
	{
		...
		case SDL_GAMEPAD_LEFT_TRIGGER:
			...
			break;
		default:
			GetISystem()->GetILog()->LogError("CSDLPad - Unknown AXIS requested(%d) on Gamepad [%d]", axisEvt.axis, m_deviceNo);
			return;
		}
...
void CSDLPad::HandleButtonEvent(const SDL_JoyButtonEvent& btEvt)
{
	...
	if (pSymbol)
	{
		...
		case SDL_GAMEPAD_LEFT_THUMB_RIGHT:
			isPost = (pSymbol->state == eIS_Unknown) || (currState == eIS_Pressed && (pSymbol->state == eIS_Released)) || (currState == eIS_Released && (pSymbol->state == eIS_Pressed || pSymbol->state == eIS_Down));
			break;
		default:
			isPost = true;
			break;
	...
	else
	{
		...
		case SDL_GAMEPAD_BUTTON_RIGHT_TRIGGER_DUPE:
			break;
		default:
			GetISystem()->GetILog()->LogError("CSDLPad - Gamepad [%d] received unmapped button event %d", m_deviceNo, btEvt.button);
			break;
		}
	}
}
...
void CSDLPad::HandleConnectionState(const bool connected)
{
	if (m_connected != connected)
	{
		...
		event.deviceIndex = (uint8_t)m_deviceNo;
		...
		else
		{
			if (OpenDevice())
			{
				...
				GetLinuxInput().PostInputEvent(event);
				GetISystem()->GetILog()->Log("CSDLPad - Gamepad [%d] connected (%s)", m_deviceNo, SDL_JoystickName(m_pSDLDevice));
			}
			else
			{
				GetISystem()->GetILog()->LogError("CSDLPad - Could not connect to Gamepad [%d]", m_deviceNo);
			}
		}
	}
}
...
#endif
		m_curHapticEffect = -1;
		GetISystem()->GetILog()->Log("CSDLPad - Gamepad [%d] supports feedback (Y/N): %s", m_deviceNo, m_supportsFeedback ? "Y" : "N");
	}
...
CSDLPadManager::~CSDLPadManager()
{
	...
	{
		//Attention - double deleteion, begin here, after in BaseInput!
		//delete* it;
	}
}
...
bool CSDLPadManager::Init()
{
	...
	{
		GetISystem()->GetILog()->LogError("CSDLPadManager - SDL_SetHint SDL_HINT_ACCELEROMETER_AS_JOYSTICK failed: %s", SDL_GetError());
	}
#endif
	// setup devices
...
void CSDLPadManager::Update(bool bFocus)
{
	...
	if (nEvents == -1)
	{
		GetISystem()->GetILog()->LogError("CSDLPadManager - SDL_GetEvent error: %s", SDL_GetError());
		return;
	}
	for (int i = 0; i < nEvents; ++i)
	{
		...
		if (type == SDL_JOYAXISMOTION)
		{
			...
			if (id < 0)
			{
				GetISystem()->GetILog()->LogError("CSLDPadManager - Received event from invalid Gamepad [%d]", axisEvt.which);
				continue;
			}
		...
		else if (type == SDL_JOYHATMOTION)
		{
			...
			if (id < 0)
			{
				GetISystem()->GetILog()->LogError("CSLDPadManager - Received event from invalid Gamepad [%d]", hatEvt.which);
				continue;
			}
			CSDLPad* pad = FindPadByInstanceId(id);
			if (pad != NULL)
		...
		else if (type == SDL_JOYBUTTONDOWN || type == SDL_JOYBUTTONUP)
		{
			...
			if (id < 0)
			{
				GetISystem()->GetILog()->LogError("CSLDPadManager - Received event from invalid Gamepad [%d]", btEvt.which);
				continue;
			}
			CSDLPad* pad = FindPadByInstanceId(id);
			if (pad != NULL)
		...
		else if (type == SDL_JOYBALLMOTION)
		{
			...
			GetISystem()->GetILog()->LogError("CSLDPadManager - Gamepad [%d] triggered a ball motion event which is not supported", ballEvt.which);
		}
		else if (type == SDL_JOYDEVICEREMOVED)
		{
			...
			{
				GetISystem()->GetILog()->LogError("CSLDPadManager - Failed to remove Gamepad [%d]", devEvt.which);
			}
		}
		else if (type == SDL_JOYDEVICEADDED)
		{
			...
			{
				GetISystem()->GetILog()->LogError("CSLDPadManager - Failed to Add Gamepad [%d]", devEvt.which);
			}
		}
		else
		{
			GetISystem()->GetILog()->LogError("CSDLPadManager - Unknown event processed: %x", type);
			break;
		}
	}
}
...
CSDLPad* CSDLPadManager::FindPadByInstanceId(int instanceId)
{
	...
	GetISystem()->GetILog()->LogError("CSLDPadManager - Unknown device instance [%d]", instanceId);
	return NULL;
}
